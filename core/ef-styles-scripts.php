...
$styles_scripts->addField('thirdparty', 'fullpage', __('FULLPAGE', 'ef'), null, 'checkbox', true, array('checkboxText' => __('ENABLE', 'ef')));
...
function enqueue_styles_and_scripts() {
    ...
    if(!is_admin()) {
        ...
        if(isset($options['swiper'])) {
            ...
            wp_register_script('swiper', 'https://unpkg.com/swiper/swiper-bundle.min.js', array('jquery'));
        }

        # fullpage
        if(isset($options['fullpage'])) {
            wp_register_script('fullpage', 'https://cdnjs.cloudflare.com/ajax/libs/fullPage.js/2.9.7/jquery.fullpage.min.js', array('jquery', 'ef-frontendJS'));
...
function enqueue_admin_styles_and_scripts() {
    ...
    # theme admin js
    wp_register_script('vue','https://cdn.jsdelivr.net/npm/vue@2.x/dist/vue.js', '', '', true);
    wp_enqueue_script( 'vue' );

    # theme admin js
    wp_register_script('vuetify','https://cdn.jsdelivr.net/npm/vuetify@2.x/dist/vuetify.js', '', '', true);
    wp_enqueue_script( 'vuetify' );

    # theme admin CSS
    wp_register_style('ef-backend', get_template_directory_uri().'/assets/css/backend.css');
